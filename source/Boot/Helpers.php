...
function str_limit_words(string $string, int $limit, string $pointer): string {
    ...
    if(mb_strlen($numWords) <= $limit) {
        return $string;
    }

    $words = implode(" ", array_slice($arrWords, 0, $pointer));
...
function url_image(string $path = null): string {
    if($path) {
        return __DIR__ . "/../../assets/img/" . ($path[0] == "/" ? substr($path, 1) : $path);
    }

    return __DIR__ . "/../../assets/img";
}
...
/**
 * Imprime um json e mata a aplicação
 *
 * @param mixed $value
 */
function emit_json($value): void {
